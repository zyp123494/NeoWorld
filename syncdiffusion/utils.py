...
def get_views(
    panorama_height, panorama_width, window_size=64, stride=8, loop_closure=False
):
    panorama_height /= 8
    panorama_width /= 8
...
def set_latent_view(latent, h_start, h_end, w_start, w_end):
    ...
    if w_end > w_start:
        ...
        latent_view = torch.cat(
            [latent_view, latent[:, :, h_start:h_end, :w_end]], dim=-1
        )
...
def set_image_view(image, h_start, h_end, w_start, w_end):
    ...
    if w_end > w_start:
        ...
        image_view = torch.cat([image_view, image[:, :, h_start:h_end, :w_end]], dim=-1)
