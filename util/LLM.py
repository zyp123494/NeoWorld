...
class TextpromptGen:
    def __init__(self, root_path, control=False):
        ...
        if control:
            self.base_content = (
                "Please generate scene description based on the given information:"
            )
        else:
    ...
    def write_json(self, output, save_dir=None):
        ...
        try:
            ...
            with open(
                save_dir / f"scene_{str(self.scene_num).zfill(2)}.json", "w"
            ) as json_file:
    ...
    def wonder_next_scene(
        ...
        if change_scene_name_by_user:
            ...
            assert self.scene_num > 0, (
                "To regenerate the scene description, you should have at least one scene content as prompt."
            )
        ...
        if change_scene_name_by_user:
            messages = [
                {
                    "role": "system",
                    "content": "You are an intelligent scene generator. Imaging you are wandering through a scene or a sequence of scenes, and there are 3 most significant common entities in each scene. The next scene you would go to is "
                    + scene_name
                    + ". Please generate the correspondant 3 most common entities in this scene. The scenes are sequentially interconnected, and the entities within the scenes are adapted to match and fit with the scenes. You also have to generate a brief background prompt about 50 words describing the scene. You should not mention the entities in the background prompt. If needed, you can make reasonable guesses. Please use the format below: (the output should be json format)\n \
                ...
                try:
                    ...
                    if "```json" in response:
                        response = response.split("```json")[
                            1
                        ]  # Remove ```json and its preceding content
                    if "```" in response:
                        response = response.split("```")[
                            0
                        ]  # Remove ``` and its following content
    ...
    def evaluate_image(self, image, eval_blur=True, eval_partial=False):
        ...
        payload["messages"][0]["content"][0]["text"] = (
            border_text + " Your answer should be simply 'Yes' or 'No'."
        )
        for i in range(5):
            try:
                ...
                print(
                    "Something has been wrong while asking GPT4V. Wait for a second and ask chatGPT4 again!"
                )
                time.sleep(1)
                continue
        ...
        if eval_blur:
            ...
            print(blur_text)
            payload["messages"][0]["content"][0]["text"] = (
                blur_text + " Your answer should be simply 'Yes' or 'No'."
            )
            for i in range(5):
                try:
                    response = requests.post(
        ...
        if eval_partial:
            ...
            print(partial_text)
            payload["messages"][0]["content"][0]["text"] = (
                partial_text + " Your answer should be simply 'Yes' or 'No'."
            )
            for i in range(5):
                try:
                    response = requests.post(
