...
class StableDiffusionInpaintPipeline(
    ...
    def __call__(
        ...
        image: torch.FloatTensor | PIL.Image.Image = None,
        mask_image: torch.FloatTensor | PIL.Image.Image = None,
        ...
        inpaint_mask: torch.FloatTensor | None = None,
        rendered_image: torch.FloatTensor | None = None,
    ):
        r"""
            ...
            for i, t in enumerate(timesteps):
                if self_guidance:
                    with torch.set_grad_enabled(self_guidance):
                        ...
                        latents_orig = self.scheduler.step(
                            noise_pred,
                            t,
                            latents,
                            **extra_step_kwargs,
                        ...
                        if num_channels_unet == 4:
                            ...
                            latents_orig = (
                                1 - init_mask
                            ) * init_latents_proper + init_mask * latents_orig
                        ...
                        image_orig = self.vae.decode(
                            latents_orig / self.vae.config.scaling_factor,
                            return_dict=False,
                            **condition_kwargs,
                        )[0]
                        image_orig = (
                            (image_orig / 2 + 0.5).clamp(0, 1).to(torch.float32)
                        )
                        ...
                        print(
                            f"-- {i} noise {(noise_pred**2).mean()} grad {(sg_grad**2).mean()} noise/grad {(noise_pred**2).mean() / (sg_grad**2).mean()}"
                        )

                    l2 = ((image_orig - rendered_image) ** 2 * inpaint_mask)[
                        inpaint_mask > 0
                    ].mean()
                    print(
                        f"---- unet original {i}: l2 {l2} l1 {(torch.abs(image_orig - rendered_image) * inpaint_mask)[inpaint_mask > 0].mean()}"
                    )
                    ...
                    l2 = ((image - rendered_image) ** 2 * inpaint_mask)[
                        inpaint_mask > 0
                    ].mean()
                    print(
                        f"---- unet self_guidance {i}: l2 {l2} l1 {(torch.abs(image - rendered_image) * inpaint_mask)[inpaint_mask > 0].mean()}"
                    )
