...
from utils.graphics import focal2fov, fov2focal, getProjectionMatrix, getWorld2View2
...
def loadCamerasFromData(traindata, white_background):
    ...
    for idx, frame in enumerate(frames):
        ...
        panoptic_mask = frame["panoptic_mask"] if "panoptic_mask" in frame else None
        im_data = np.array(image.convert("RGBA"))
        ...
        if panoptic_mask is not None:
            panoptic_mask = torch.Tensor(panoptic_mask)
