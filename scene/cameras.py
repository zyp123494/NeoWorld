...
class Camera(nn.Module):
    def __init__(
        ...
        self.panoptic_mask = panoptic_mask
        ...
        if no_loss_mask is None:
            no_loss_mask = torch.zeros_like(
                self.original_image
            ).bool()  # Do not remove image
        ...
        self.K = torch.tensor(
            [
                [self.focal_x, 0, self.image_width / 2],
                [0.0, self.focal_y, self.image_height / 2],
                [0.0, 0.0, 1.0],
            ],
