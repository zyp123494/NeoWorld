    ...
    if DEBUG:
        if s is not None:
            for i in range(s.shape[0]):
                assert (s.coords[s.layout[i]] == i).all(), (
                    "SparseScaledDotProductSelfAttention: batch index mismatch"
                )
        if num_all_args in [2, 3]:
            assert q.shape[:2] == [1, sum(q_seqlen)], (
                "SparseScaledDotProductSelfAttention: q shape mismatch"
            )
        if num_all_args == 3:
            assert k.shape[:2] == [1, sum(kv_seqlen)], (
                "SparseScaledDotProductSelfAttention: k shape mismatch"
            )
            assert v.shape[:2] == [1, sum(kv_seqlen)], (
