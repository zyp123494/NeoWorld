...
def yaw_pitch_r_fov_to_extrinsics_intrinsics(yaws, pitchs, rs, fovs):
    ...
    extrinsics = []
    intrinsics = []
    for yaw, pitch, r, fov in zip(yaws, pitchs, rs, fovs, strict=False):
...
def render_frames(
    ...
    **kwargs,
    ...
    for j, (extr, intr) in tqdm(
        enumerate(zip(extrinsics, intrinsics, strict=False)),
        desc="Rendering",
        disable=not verbose,
    ):
...
def render_multiview(sample, resolution=512, nviews=30, bg_color=(0, 0, 0)):
    ...
    res = render_frames(
        sample, extrinsics, intrinsics, {"resolution": resolution, "bg_color": bg_color}
    )
